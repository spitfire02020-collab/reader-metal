...
public actor KVCacheManager {
    ...
    /// Marked nonisolated(unsafe) because buffer contents are immutable references
    /// (only memory contents change via memcpy, not the buffer references themselves).
    private nonisolated(unsafe) var layerBuffers: [KVCacheBufferSet]
    ...
    public nonisolated func buffer(for layer: Int, isKey: Bool) -> MTLBuffer {
