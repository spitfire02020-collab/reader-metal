...
final class ONNXLMBackend: LanguageModelBackend {
    ...
    public func forward(
        ...
        case .float16:
            ...
            let logitsPtr16 = logitsRawData.withUnsafeBytes { ptr -> UnsafePointer<UInt16> in
                ptr.assumingMemoryBound(to: UInt16.self)
            }
            for i in 0..<vocabSize {
                outPtr32[i] = float16ToFloat32(logitsPtr16[lastRowStart + i])
            }

        case .float:
            // Raw bytes are float32; copy directly.
            let logitsPtr32 = logitsRawData.withUnsafeBytes { ptr -> UnsafePointer<Float> in
                ptr.assumingMemoryBound(to: Float.self)
            }
            for i in 0..<vocabSize {
                outPtr32[i] = logitsPtr32[lastRowStart + i]
            }
