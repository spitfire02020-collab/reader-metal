...
// GroupQueryAttention kernels for chatterbox-metal-lm
// NUM_Q_HEADS=80, NUM_KV_HEADS=16, HEAD_DIM=64
// Q/K/V heads: 5 Q heads share 1 KV head (80/16=5)
...
constant int Q_KV_RATIO = NUM_Q_HEADS / NUM_KV_HEADS;  // 5
...
kernel void attention_decode_step(
    device const half* q       [[buffer(0)]],  // [NUM_Q_HEADS, HEAD_DIM] = [80, 64]
    ...
    device half*         output  [[buffer(3)]],  // [NUM_Q_HEADS, HEAD_DIM] = [80, 64]
...
)
{
    ...
    // Each group of Q_KV_RATIO=5 Q heads shares 1 KV head
    uint kv_head = q_head / Q_KV_RATIO;  // q_head / 5
    ...
    for (uint pos = 0; pos < kv_len; ++pos) {
        ...
        // Compute partial dot product (only this dimension)
        half k_val = k[k_idx];
...
kernel void group_query_attention(
    device const half* Q       [[buffer(0)]],  // [1, NUM_Q_HEADS, seq, HEAD_DIM] = [1, 80, seq, 64]
    ...
    device float*       output  [[buffer(3)]],  // [1, NUM_Q_HEADS, seq, HEAD_DIM] = [1, 80, seq, 64]
